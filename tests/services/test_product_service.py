...
class TestCreateProduct:
    ...
    def test_create_product_success(
        ...
        product = ProductService.create_product(
            name=name, price=price, stock=stock, db=test_db, redis=redis_client
        )
...
class TestGetProduct:
    ...
    def test_get_product_exists(self, test_db: Session, redis_client: Redis):
        """Test: 존재하는 상품 조회"""
        # 상품 생성
        product = ProductService.create_product(
            name="Test Product",
...
class TestListProducts:
    ...
    def test_list_products_multiple(self, test_db: Session, redis_client: Redis):
        """Test: 여러 상품 조회"""
        # 3개 상품 생성
        ProductService.create_product(
            name="Product 1", price=10000, stock=5, db=test_db, redis=redis_client
        )
        ProductService.create_product(
            name="Product 2", price=20000, stock=10, db=test_db, redis=redis_client
        )
        ProductService.create_product(
            name="Product 3", price=30000, stock=15, db=test_db, redis=redis_client
        )
    ...
    def test_list_products_limit_only(self, test_db: Session, redis_client: Redis):
        """Test: limit만 지정"""
        # 5개 상품 생성
        for i in range(1, 6):
            ProductService.create_product(
