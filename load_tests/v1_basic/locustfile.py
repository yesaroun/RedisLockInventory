"""
V1 Basic Scenario - Locust Load Test

시나리오: 100명이 동시에 1개씩 구매 (총 100개 재고)
...
실행 방법:
  ...
  # 헤드리스 모드 (100명, 60초)
  locust -f load_tests/v1_basic/locustfile.py --headless --users 100 --spawn-rate 10 -t 60s --host=http://localhost:8080
"""
...
class InventoryTaskSet(TaskSet):
    """V1 Basic 시나리오 사용자 행동 모델"""

    def on_start(self):
        """각 사용자가 시작할 때 실행: 회원가입 및 로그인"""
        self.user_id = f"v1_basic_user_{random.randint(1, 1000000)}"
    ...
    def _register(self):
        ...
        with self.client.post(
            ...
            json={
                ...
                "password": "test1234",
                "email": f"{self.user_id}@v1basic.com",
            },
            name="[Auth] Register",
            catch_response=True,
        ) as response:
...
class V1BasicUser(HttpUser):
    """V1 Basic 시나리오 일반 사용자 (일반적인 쇼핑 행동)"""
...
def on_test_stop(environment, **kwargs):
    ...
    print("\n" + "=" * 60)
    print("📊 V1 Basic Scenario - Test Results")
    print("=" * 60)
