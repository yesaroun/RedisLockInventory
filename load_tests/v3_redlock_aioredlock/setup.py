#!/usr/bin/env python3
"""
V3 Redlock Aioredlock 시나리오 테스트 데이터 초기화

시나리오: 300명이 동시에 100개 재고 상품 구매 시도 (aioredlock 라이브러리 사용)
목표: 정확히 100개만 판매, 초과 판매 0건, 쿼럼 기반 분산 락 동작 검증
"""
...
def create_admin_user(base_url: str) -> str:
    ...
    register_response = requests.post(
        ...
        json={
            "username": "admin_v3_aioredlock",
            "password": "admin1234",
            "email": "admin_v3_aioredlock@loadtest.com",
        },
    )
    ...
    login_response = requests.post(
        ...
        data={"username": "admin_v3_aioredlock", "password": "admin1234"},
    )
...
def create_test_product(base_url: str, token: str) -> dict:
    """V3 Redlock Aioredlock 테스트 상품 생성: 재고 100개"""
    headers = {"Authorization": f"Bearer {token}"}

    product_name = "V3 Redlock Aioredlock Test Product"
    stock = 100
    ...
    response = requests.post(
        ...
        json={
            "name": product_name,
            "description": f"V3 Redlock Aioredlock scenario - {stock} units for 300 aggressive buyers",
            "price": 10000,
            "stock": stock,
        },
        headers=headers,
...
def main():
    parser = argparse.ArgumentParser(
        description="Setup test data for V3 Redlock Aioredlock scenario"
    )
    parser.add_argument(
        "--host",
    ...
    print("\n" + "=" * 60)
    print("V3 Redlock Aioredlock Scenario - Test Data Setup")
    print("=" * 60)
    print(f"Target: {args.host}")
    print(f"Scenario: 300 aggressive buyers → 100 stock")
    print(f"Redlock: aioredlock library (5 Redis nodes, quorum 3/5)")
    print("=" * 60 + "\n")
    ...
    print("\n" + "=" * 60)
    print("✅ V3 Redlock Aioredlock Test Data Setup Complete!")
    print("=" * 60)
    print(f"\n📦 Product ID: {product['id']}")
    print(f"📊 Initial Stock: {product['stock']}")
    print(f"\n🔒 Distributed Lock: aioredlock library")
    print(f"📡 Redis Nodes: 5 (quorum 3/5)")
    print("\nYou can now run the Locust test:")
    print(f"  locust -f load_tests/v3_redlock_aioredlock/locustfile.py --host={args.host}")
    print("\nOr headless mode:")
    print(
        f"  locust -f load_tests/v3_redlock_aioredlock/locustfile.py --headless --users 300 --spawn-rate 10 -t 60s --host={args.host}"
    )
    print("=" * 60 + "\n")
