"""
V3 Redlock Aioredlock Scenario - Locust Load Test

시나리오: 300명이 동시에 100개 재고를 경쟁 (aioredlock 라이브러리 사용)

특징:
- aioredlock 라이브러리 기반 Redlock 알고리즘
- 5개 Redis 노드에 분산 락 획득
- 쿼럼(3/5) 기반 합의 알고리즘
- 높은 가용성 및 정합성 보장
...
실행 방법:
  ...
  # 헤드리스 모드 (300명, 60초)
  locust -f load_tests/v3_redlock_aioredlock/locustfile.py --headless --users 300 --spawn-rate 10 -t 60s --host=http://localhost:8080
"""
...
class RedlockAioredlockTaskSet(TaskSet):
    """V3 Redlock Aioredlock 시나리오 사용자 행동 모델"""

    def on_start(self):
        """각 사용자가 시작할 때 실행: 회원가입 및 로그인"""
        self.user_id = f"v3_aioredlock_user_{random.randint(1, 1000000)}"
    ...
    def _register(self):
        ...
        with self.client.post(
            ...
            json={
                ...
                "password": "test1234",
                "email": f"{self.user_id}@v3aioredlock.com",
            },
            name="[Auth] Register",
            catch_response=True,
        ) as response:
    ...
    @task(10)
    def purchase_product_redlock_aioredlock(self):
        """상품 구매 (aioredlock 라이브러리 기반 Redlock)"""
        if not self.product_id:
            # 상품 ID가 없으면 먼저 목록 조회
            self.list_products()
            if not self.product_id:
        ...
        with self.client.post(
            "/api/purchases/redlock-aioredlock",
            json={"product_id": self.product_id, "quantity": 1},
            headers=self._get_headers(),
            name="[Purchase] Buy Product (Redlock Aioredlock)",
            catch_response=True,
        ) as response:
            if response.status_code == 201:
                total_purchases += 1
...
class V3RedlockAioredlockUser(HttpUser):
    """V3 Redlock Aioredlock 시나리오 공격적 구매자 (블랙프라이데이)"""

    tasks = [RedlockAioredlockTaskSet]
...
def on_test_start(environment, **kwargs):
    ...
    print("\n" + "=" * 60)
    print("🚀 V3 Redlock Aioredlock Scenario - Load Test Started")
    print("=" * 60)
    ...
    print("Redlock: aioredlock library (5 Redis nodes, quorum 3/5)")
    print("=" * 60 + "\n")
...
def on_test_stop(environment, **kwargs):
    ...
    print("\n" + "=" * 60)
    print("📊 V3 Redlock Aioredlock Scenario - Test Results")
    print("=" * 60)
    ...
    # 초과 판매 검증 (V3 목표: 0건)
    if oversold_count > 0:
        print("❌ FAIL: Overselling detected! Redlock has bugs.")
    else:
        print("✅ PASS: No overselling detected. Redlock works correctly!")
